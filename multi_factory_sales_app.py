...
st.title("🏭 Multi-Unit Sales Dashboard with Forecasting")

units = ["Unit 1", "Unit 2", "Unit 3", "Unit 4"]
tabs = st.tabs(units)

def render_dashboard(tab, unit_name):
    with tab:
        st.header(f"📦 {unit_name} Dashboard")
        uploaded_zip = st.file_uploader(f"Upload ZIP file for {unit_name}", type="zip", key=unit_name)
        ...
        if uploaded_zip:
            with zipfile.ZipFile(uploaded_zip) as z:
                ...
                if len(dfs) < 2:
                    ...
                    selected_month = st.selectbox("Select Month to View Sales Data", month_options, index=len(month_options)-1, key=unit_name+"month")
                    filtered_data = combined_df[combined_df["Month"] == selected_month]

                    product_filter = st.text_input("Search Product Name (optional)", key=unit_name+"product")
                    if product_filter:
                    ...
                    st.download_button("📤 Download Current Month Data", data=filtered_data.to_csv(index=False), file_name=f"{unit_name.lower().replace(' ','_')}_sales_data_{selected_month.replace(' ', '_')}.csv")
                    ...
                    st.download_button("📥 Download Comparison CSV", data=merged.to_csv(index=False), file_name=f"{unit_name.lower().replace(' ','_')}_comparison.csv")
                    ...
                    st.download_button("📥 Download Monthly Sales Summary", data=monthly_summary.to_csv(index=False), file_name=f"{unit_name.lower().replace(' ','_')}_monthly_summary.csv")

                    st.subheader("📊 Product-wise Monthly Trend")
                    selected_prod = st.selectbox("Select Product for Trendline", sorted(combined_df["Product_Name"].unique()), key=unit_name+"trend")
                    ...
                    selected_forecast_prod = st.selectbox("Product for Forecast", sorted(history["Product_Name"].unique()), key=unit_name+"forecast")
                    ...
                    st.download_button("📥 Download Forecast (Selected Product)", forecast_df.to_csv(index=False), file_name=f"{selected_forecast_prod}_forecast_{unit_name}.csv")
                    ...
                    st.download_button("📥 Download Forecast Summary (All Products)", data=forecast_summary_df.to_csv(index=False), file_name=f"{unit_name.lower().replace(' ','_')}_forecast_summary.csv")
            ...
            st.info(f"📤 Please upload a ZIP file with Excel sheets named like 'sales_april_2025.xlsx' for {unit_name}.")

# Call render_dashboard for each unit tab
for tab, name in zip(tabs, units):
    render_dashboard(tab, name)
