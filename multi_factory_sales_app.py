...
st.title("🏭 Multi-Unit Monthly Sales Dashboard")

units = {
    "Unit 1": st.file_uploader("Upload ZIP for Unit 1", type="zip", key="unit1"),
    "Unit 2": st.file_uploader("Upload ZIP for Unit 2", type="zip", key="unit2"),
    "Unit 3": st.file_uploader("Upload ZIP for Unit 3", type="zip", key="unit3"),
    "Unit 4": st.file_uploader("Upload ZIP for Unit 4", type="zip", key="unit4")
}

tabs = st.tabs(list(units.keys()))

for i, (unit, uploaded_zip) in enumerate(units.items()):
    with tabs[i]:
        if uploaded_zip:
                    ...
                    if name.endswith(".xlsx"):
                        ...
                        if {"Product_Name", "Quantity_Sold", "Sales_Value"}.issubset(df.columns):
                            try:
                                ...
                                st.warning(f"⚠️ Could not parse date from: {name}")
                        else:
                            st.warning(f"❌ Skipping '{name}' — missing columns.")

                if len(dfs) < 2:
                    st.error("❗ Upload at least 2 valid monthly sheets.")
                else:
                    ...
                    selected_month = st.selectbox("Select Month", month_options, index=len(month_options)-1, key=f"{unit}_month")
                    filtered_data = combined_df[combined_df["Month"] == selected_month]

                    product_filter = st.text_input("Search Product Name (optional)", key=f"{unit}_search")
                    if product_filter:
                    ...
                    st.download_button("📤 Download Month Data", data=filtered_data.to_csv(index=False),
                                       file_name=f"{unit}_sales_{selected_month.replace(' ', '_')}.csv")
                    ...
                    st.download_button("📥 Download Comparison", merged.to_csv(index=False), file_name=f"{unit}_comparison.csv")
                    ...
                    monthly_summary = monthly_summary[["Month", "Quantity_Sold", "Sales_Value", "MoM_Growth_Quantity_%", "MoM_Growth_Sales_Value_%"]].round(2)
                    ...
                    st.download_button("📥 Download Summary", monthly_summary.to_csv(index=False), file_name=f"{unit}_summary.csv")

                    # Forecast Summary Table (All Products)
                    st.subheader("📦 Forecast Summary (Next Month)")
                    ...
                    AgGrid(forecast_df)
                    st.download_button("📥 Download Forecast Summary", forecast_df.to_csv(index=False), file_name=f"{unit}_forecast_summary.csv")

                    if not forecast_df.empty:
                        st.markdown(f"### 💡 Total Forecasted Sales (30 Days): ₹{forecast_df['Forecasted_Sales_Value'].sum():,.2f}")

                    # Product Trend Graph
                    st.subheader("📈 Product Trend Graph")
                    selected_prod = st.selectbox("Select Product for Trendline", sorted(combined_df["Product_Name"].unique()), key=f"{unit}_trend")
                    ...
                    ax.set_title(f"{selected_prod} Sales Trend")
                    ax.set_ylabel("Amount")
                    ax.legend()
                    st.pyplot(fig)

                    # Forecast Plot for Selected Product
                    st.subheader("📈 30-Day Forecast (Selected Product)")
                    selected_forecast_prod = st.selectbox("Select Product for 30-Day Forecast", sorted(history["Product_Name"].unique()), key=f"{unit}_forecast")
                    ...
                    if len(hist) >= 2:
                        ...
                        forecast_qty = model.predict(future_ordinals)

                        fig, ax = plt.subplots(figsize=(10, 4))
                        ax.plot(hist["Date"], hist["Quantity_Sold"], label="Historical", marker='o')
                        ax.plot(future_dates, forecast_qty, label="Forecast", linestyle='--', marker='x')
                        ax.set_title(f"30-Day Quantity Forecast: {selected_forecast_prod}")
                        ax.set_ylabel("Quantity")
                        ax.legend()
                        st.pyplot(fig)

                        forecast_detail_df = pd.DataFrame({"Date": future_dates, "Predicted_Quantity": forecast_qty})
                        st.download_button("📥 Download 30-Day Forecast", forecast_detail_df.to_csv(index=False), file_name=f"{unit}_{selected_forecast_prod}_30day_forecast.csv")
                    else:
                        st.info("Need at least 2 months of data to generate a forecast.")

        else:
            st.info("📤 Upload ZIP for this unit to begin.")
